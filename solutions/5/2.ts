...
const seeds = [[3121711159,166491471]];
...
seeds.forEach((seed) => {
  ...
  for (let iterator = seed[0]; iterator <= (seed[0] + seed[1]); iterator++) {    
    allSeedValues.push(iterator);
  }

  // console.log(allSeedValues);
  

  allSeedValues.forEach((newSeed) => {
    const soil = checker(data.seedToSoilMap, newSeed);
